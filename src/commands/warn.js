    ...
    try {
      ...
      if (!args.reason) {
        ...
        const embed = errorMessage(
          ...
          'Please specify a' + ' reason for the warning.'
        )
        return message.util.send({ embed })
      }
      ...
      try {
        ...
        await message.guild.channels.find(c => c.name === channels.warn).send({
          embed: {
        ...
        if (autoBan && row.count >= autoBanWarns) {
          ...
          await db
            .prepare('DELETE FROM warnings WHERE discord_id = ?')
            .run(user.id)
          await message.guild.channels.find(c => c.name === channels.ban).send({
            embed: {
              title: 'Auto Ban',
              color,
              ...
              footer: {
                ...
                text: `${
                  user.tag
                } banned. Messages from past ${banMsgDelete} days deleted.`
              },
              timestamp: new Date()
            }
          })
